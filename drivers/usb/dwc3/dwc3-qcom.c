...
/**
 * struct dwc3_qcom_data - per-compatible quirks
 * @defer_utmi_clk_sel: on this platform the QSCRATCH register interface is
 *	clocked by the eUSB2 PHY's UTMI clock, so the select-utmi-as-pipe-clk
 *	configuration must be written only after the PHY has been initialized.
 */
...
static int dwc3_qcom_probe(struct platform_device *pdev)
{
	...
	struct resource		res;
	struct resource		*r;
	const struct dwc3_qcom_data *data;
	int			ret;
	...
	/*
	 * Assert and deassert the block reset only after the clocks are
	 * running: the clock-domain synchronizers inside the USB tile need
	 * their clocks present for the reset release to propagate. A reset
	 * deasserted with the clocks still gated can leave the controller
	 * register interface unresponsive.
	 */
	...
	if (ignore_pipe_clk && !(data && data->defer_utmi_clk_sel))
		dwc3_qcom_select_utmi_clk(qcom);
