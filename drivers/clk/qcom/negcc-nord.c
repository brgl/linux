...
static struct clk_branch ne_gcc_usb3_prim_phy_aux_clk = {
	.halt_reg = 0x2a06c,
	/*
	 * The aux clocks are sourced from an RCG whose root only runs while
	 * the combo PHY's SerDes PLL is locked, which happens after the clock
	 * framework has already enabled the branch. Polling CLK_OFF here
	 * therefore always times out; skip it as for the pipe clocks.
	 */
