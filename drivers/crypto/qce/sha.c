...
/*
 * The crypto engine's BAM DMA cannot handle zero-length transfers. For plain
 * hashes the result of an empty message is a known constant (hash_zero); for
 * keyed HMAC it depends on the key, so compute it with the software fallback.
 */
				 ...
				 unsigned int keylen)
{
	 ...
	 * messages, which the crypto engine's DMA cannot process.
	 */
...
static int qce_ahash_hmac_cra_init(struct crypto_tfm *tfm)
{
	 ...
	 * crypto engine's DMA cannot process. Exclude the crypto engine's own
	 * algorithm (which carries CRYPTO_ALG_NEED_FALLBACK) to avoid recursion.
	 */
