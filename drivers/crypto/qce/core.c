...
static int qce_crypto_probe(struct platform_device *pdev)
{
	...
	if (device_property_read_bool(dev, "qce,cmd_desc_support")) {
		...
		pr_err("##deugudit## cmd desc support true");
		res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
		if (!res)
			return -EINVAL;
		/* Store physical base address for BAM command descriptor register access */
		qce->phy_base = res->start;

		/* Create DMA mapping for register space to enable BAM access */		
